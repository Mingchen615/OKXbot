...
class AIClient:
    ...
    async def _fallback_fusion(self, market_data: Dict[str, Any]) -> tuple:
        """备用融合方案 - 当主提供商失败时使用"""
        record_fallback_invocation()
        preferred_order = ["gemini", "qwen", "openai", "deepseek", "kimi", "minimax", "mimo"]
        seen = set()
        fallback_providers = []
