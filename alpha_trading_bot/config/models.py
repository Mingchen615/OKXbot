...
class AIConfig:
    ...
    fusion_weights: Dict[str, float] = field(
        default_factory=lambda: {"deepseek": 0.5, "kimi": 0.5, "mimo": 0.5}
    )
    ...
    VALID_PROVIDERS = ["deepseek", "kimi", "openai", "qwen", "gemini", "minimax", "mimo"]
    VALID_STRATEGIES = [
        "weighted",
        "majority",
        "consensus",
    ...
    def from_env(cls) -> "AIConfig":
        ...
        fusion_providers_str = os.getenv("AI_FUSION_PROVIDERS", "deepseek,kimi,mimo")
        fusion_providers = [
            provider.strip()
        ...
        if not fusion_providers:
            fusion_providers = ["deepseek", "kimi", "mimo"]

        fusion_weights_str = os.getenv("AI_FUSION_WEIGHTS", "deepseek:0.5,kimi:0.5,mimo:0.5")
