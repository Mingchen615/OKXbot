...
class ExchangeClient:
    ...
    async def initialize(self) -> None:
        ...
        # TEST_MODE=true 时保持订单模拟保护（create_order/create_stop_loss/create_take_profit
        # 会跳过真实下单），但不切换到沙盒URL，避免API Key不匹配导致连接失败
        if self.test_mode:
            logger.info("交易所模式: 实盘API连接 + 订单模拟保护 (TEST_MODE=true)")
        else:
            logger.info("交易所模式: 实盘连接 (TEST_MODE=false)")
