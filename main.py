...
async def main() -> None:
    ...
    # Web界面支持
    web_enabled = args.web or os.getenv("WEB_INTERFACE_ENABLED", "false").lower() == "true"
    web_port = args.port or int(os.getenv("WEB_PORT", "8501"))

    try:
        if web_enabled:
            from alpha_trading_bot.web.server import start_web_server
            # 先初始化bot
            await bot.initialize()
            # 启动web server（后台任务）
            web_task = asyncio.create_task(start_web_server(bot, port=web_port))
            # 运行bot主循环
            await bot.run()
        else:
            await bot.run()
